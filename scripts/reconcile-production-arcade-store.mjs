/**
 * Applies only the reviewed 0036 Arcade store migration to exact production
...
const PACKET = {
  version: "20260804035000",
  name: "arcade_store_purchases",
  source: "0036_arcade_store_purchases.sql",
  sourceSha:
    "66910b82e3f5056d863d143d6f77afc84086b66c8aa93b01e1aaa0436ac3de2d",
};
...
/** Verifies the frozen production prefix and immutable 0036 source. */
async function verifyReleaseInputs() {
  ...
  if (sha256(source) !== PACKET.sourceSha) {
    fail("Reviewed 0036 source checksum changed");
  }
  return source.toString("utf8");
}

/** Creates an isolated long-version lane matching production history. */
async function prepareWorkdir(source) {
  const workdir = await mkdtemp(
    join(tmpdir(), "biblequest-production-arcade-store-"),
  );
  ...
  await writeFile(
    ...
    [
      'project_id = "BibleQuest-production-arcade-store"',
      "",
      "[db]",
      "major_version = 17",
      "",
...
/** Links only the disposable migration lane to the production project. */
...
/** Accepts only the reviewed pre-0036 or completely applied history. */
function historyState(actual) {
  ...
  fail("Production migration history differs from the reviewed 0036 history");
}
...
/** Requires the dry run to propose exactly the reviewed Arcade packet. */
function dryRun(workdir) {
  ...
  if (unique.length !== 1 || unique[0] !== PACKET_FILENAME) {
    fail("Dry run did not propose exactly the reviewed Arcade store packet");
  }
  return unique;
}
...
try {
  ...
  if (mode === "apply" && initialState === "applied") {
    fail("Reviewed production Arcade store migration is already applied");
  }
  if (mode === "apply") {
    applyPacket(workdir);
    if (historyState(remoteHistory(workdir)) !== "applied") {
      fail("Reviewed production Arcade store migration was not recorded");
    }
  }
  console.log(
    JSON.stringify({
