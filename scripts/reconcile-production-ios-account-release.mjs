/**
 * Dry-runs or applies only the reviewed 0037/0038 iOS account release packet
 * after immutable-history, source-hash, assertion, and physical-backup gates.
 */
...
const APPLY_CONFIRMATION = `apply ios account release to ${PROJECT_REF}`;
...
const PACKETS = [
  {
    ...
    sha: "e091801832cc58652a02dba4fdf67fa1b52b0313b6841ab162657625a0a999ca",
  },
  {
    version: "20260812010100",
    name: "stripe_projection_leases",
    source: "0038_stripe_projection_leases.sql",
    sha: "52660bd01c759ade18d3cf3fdf9631674198a267be715e03e28a9ebc7c57ba9f",
  },
];
...
/** Returns one pinned SHA-256 digest. */
function sha256(value) {
  return createHash("sha256").update(value).digest("hex");
}

/** Runs a bounded Supabase command with output captured for validation. */
...
/** Verifies the frozen prefix and both exact release sources. */
async function releaseSources() {
  ...
  return Promise.all(
    PACKETS.map(async (packet) => {
      ...
      if (sha256(source) !== packet.sha) {
        fail(`Reviewed source changed: ${packet.source}`);
      }
      return source;
    }),
  );
...
async function prepareWorkdir(sources) {
  const workdir = await mkdtemp(join(tmpdir(), "biblequest-ios-release-"));
  ...
  for (let index = 0; index < PACKETS.length; index += 1) {
    ...
    await writeFile(
      join(migrations, `${packet.version}_${packet.name}.sql`),
      `${before.trim()}\n\n${sources[index].trim()}\n\n${after.trim()}\n`,
    );
  }
  return workdir;
...
function history(workdir) {
  const result = json(
    supabase(
      ["migration", "list", "--linked", "--workdir", workdir, "--output-format", "json"],
      workdir,
    ),
    "Migration list",
  );
...
function historyState(actual) {
  ...
  const applied = [...reviewed, ...PACKETS.map((packet) => packet.version)];
...
function recentBackup(workdir) {
  const result = json(
    supabase(["backups", "list", "--project-ref", PROJECT_REF, "--output-format", "json"], workdir),
    "Backup list",
  );
  const backup = (result.backups ?? [])
    .filter((item) => item.status === "COMPLETED" && item.is_physical_backup === true)
...
/** Requires the dry run to propose exactly the two reviewed packets. */
function dryRun(workdir) {
  ...
  const files = [...new Set([...output.matchAll(/\b(20\d{12})_[a-z0-9_]+\.sql\b/g)].map((match) => match[0]))];
  const expected = PACKETS.map((packet) => `${packet.version}_${packet.name}.sql`);
  if (JSON.stringify(files) !== JSON.stringify(expected)) {
    fail("Dry run did not propose exactly the reviewed iOS release packets");
  }
  return files;
}
...
if (mode === "apply" && process.env.BIBLEQUEST_PRODUCTION_MIGRATION_CONFIRM !== APPLY_CONFIRMATION) {
  fail("Production apply confirmation is missing");
}

let workdir;
try {
  workdir = await prepareWorkdir(await releaseSources());
  supabase(["link", "--project-ref", PROJECT_REF, "--workdir", workdir, "--output-format", "json"], workdir);
  const state = historyState(history(workdir));
  const backupAt = recentBackup(workdir);
  const proposed = state === "reviewed" ? dryRun(workdir) : [];
  if (mode === "apply") {
    if (state === "applied") fail("Reviewed release packets are already applied");
    supabase(["db", "push", "--linked", "--workdir", workdir, "--yes"], workdir);
    if (historyState(history(workdir)) !== "applied") {
      fail("Production did not record both reviewed release packets");
    }
  }
