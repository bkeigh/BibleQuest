/**
 * Builds an isolated, forward-only Supabase lane for the frozen production
 * history and either proves or applies the reviewed 0030 entitlement packet.
 */
...
const PACKET_VERSION = "20260728203000";
const PACKET_NAME = "operator_plus_grants";
const PACKET_FILENAME = `${PACKET_VERSION}_${PACKET_NAME}.sql`;
const EXPECTED_SOURCE_SHA256 =
  "23dad492e8c1a0e1bcd1e6bb662c32ded7eb82cd856ea2c77d17d1f778450250";
...
async function prepareWorkdir() {
  const workdir = await mkdtemp(
    join(tmpdir(), "biblequest-production-operator-plus-"),
  );
  ...
  await writeFile(
    ...
    [
      'project_id = "BibleQuest-production-operator-plus"',
      "",
      "[db]",
      "major_version = 17",
      "",
  ...
  if (sourceSha !== EXPECTED_SOURCE_SHA256) {
    fail("Reviewed 0030 source checksum changed");
  }
  await writeFile(
