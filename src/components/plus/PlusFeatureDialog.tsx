...
/** Explains a locked benefit at the moment of intent without leaving the page. */
export function PlusFeatureDialog({
  description,
  onClose,
  open,
  title,
}: PlusFeatureDialogProps) {
  ...
  // Trap focus, close on Escape, and restore the control that opened the dialog.
  useEffect(() => {
    if (!open) return;
      ...
      >
        <div className="flex items-start justify-between gap-4">
          ...
          <button
            type="button"
            onClick={onClose}
            aria-label="Close Plus invitation"
