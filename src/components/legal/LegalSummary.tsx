...
export const LEGAL_DOCUMENTS: Record<LegalDocumentKind, LegalDocument> = {
  ...
  terms: {
    ...
    sections: [
      ...
      {
        title: "Plus purchases and one-time support",
        body:
          "The core BibleQuest experience is free. If Plus becomes available, Checkout shows the price and billing terms before confirmation. A monthly or annual Plus subscription renews automatically at the then-disclosed price each period until canceled. You may cancel through the Stripe billing portal; cancellation normally stops the next renewal and access continues through the already-paid period unless Stripe states otherwise. Lifetime Plus is a one-time purchase with no renewal and provides Plus access for the lifetime of the BibleQuest service and the purchasing account, subject to these terms. Except where applicable law requires a refund, Plus charges are not automatically refundable; contact support promptly and we will review a request based on the transaction and applicable law. A full refund, payment reversal, or unresolved or lost payment dispute may end the corresponding Plus access. A one-time “Support BibleQuest” payment is separate, voluntary, non-recurring, not tax-deductible, provides no Plus or spiritual benefit, and is subject to the same legally required rights and stated refund path.",
      },
      {
