-- Bounds opaque network/account rate-limit buckets to 48 hours after last use.
-- The longest accepted rate window is 24 hours, so the extra day preserves
-- enforcement while preventing dormant identifiers from accumulating forever.

create index provider_rate_limit_windows_updated_at_idx
  on public.provider_rate_limit_windows (updated_at);
...
begin
  ...
  delete from public.provider_rate_limit_windows
  where updated_at < claim_time - interval '48 hours';
...
-- Advances the readiness identity only after bounded retention is installed.
...
as $function$
  select pg_catalog.jsonb_build_object(
    'ok', true,
    'contract', 'biblequest_provider_rate_limit_v3'
  );
$function$;
