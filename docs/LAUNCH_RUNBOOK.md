...
| `pnpm check:production-readiness` | After the approved production push: all public schema through `0036`, including account, avatar, push, billing, lifetime entitlement, support, console, row-size hardening, operator Plus grants, guided progress, distributed provider limits, and the sealed Arcade store, plus content, health, metadata, and auth-provider checks pass; deployed controls and other manual gates remain separate |
...
Pass means all 35 checked-in migrations apply in the documented order,
...
Arcade store boundaries from `0023` through `0036` pass. Every
content-free public posture RPC reports the fixed contract identity and
`ok: true`, and the report meets the 44-table RLS gate. Supabase CLI and
...
match the frozen manifest and end at `0036`; a column probe alone is not
...
6. **Apply approved database migrations.** Database owner alone executes the
   ...
   - Checkpoint: production-targeted migration, RLS/grant, CAS posture, and
     ...
     version through `0036`. Do not continue to content until this passes.
   - Abort: any command error, unexpected row effect, RLS failure, privacy,
     ...
     limit; and `0036` adds the sealed Arcade store.
...
provider limit, and `0036` adds the sealed Arcade store.
...
| E05 | RLS catalog | Sanitized 44-table policy/function report plus account, avatar, push, billing, support, console, entitlement, and Arcade grants/contracts | Database owner | `[UTC / URL]` | OPEN |
| E06 | Isolation | Enabled: A/B and anonymous negative-test summary with no sentinel values. Guest-only: active A/B client behavior marked out of scope, plus mandatory 44-table RLS/grant and anonymous mutation-denial evidence | Database + QA | `[UTC / URL]` | OPEN |
