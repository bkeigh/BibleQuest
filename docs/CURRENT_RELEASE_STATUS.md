...
| First-use UX | PASS FOR LOCAL REVIEW | Six-step guide, curated low-risk starter pool, direct free-app launch, deferred installation prompt, and opt-in floating MyShepherd passed mobile browser review at 390×844 and 320×568 | Product/content owner reviews copy and placement |
| Automated verification | PASS FOR LOCAL SOURCE | Lint, TypeScript, 1,647-test Vitest baseline, focused post-fix tests, ten Playwright journeys, production/header build, service worker, observability, production audit, and whitespace checks passed | Rerun the full matrix against the committed candidate SHA |
...
| Production migration `0039` | PASS — APPLIED | The guarded Production dry-run reported `applied=true`, no proposed writes, exact reviewed packet/source hashes, and a completed physical backup at `2026-08-27T07:53:31.400Z` | Do not replay the migration; reconcile older runbook text |
