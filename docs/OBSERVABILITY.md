...
5. The 1,000-row CLI cap and 10 MiB export cap fail closed. A cap hit discards
   ...
   than accepting partial counts. Complete access with any missing auth, sync,
   or worker synthetic also returns `HOLD`.
...
at preflight and a hard stop from T+0 onward; truncated or incomplete synthetic
coverage is a hard stop at every phase.
