...
| Email sign-in + callback (enabled track) | Request an email for Gmail and iCloud addresses that are not Supabase organization members. Confirm the UI says the email was requested (not delivered), shows the target address, holds resend for 60 seconds, and keeps Google available. In a fresh installed PWA, enter the code in the requesting app; separately probe `/auth/callback` with a disposable `token_hash`. | Both messages have matching Supabase and SMTP-provider delivery events; the PWA code creates and retains its session inside standalone mode after a full close/reopen; email contains no portable auth link; the callback rejects `token_hash` before creating a Supabase client and remains `private, no-store`. Guest-only records provider delivery/round trips out of scope and runs the containment matrix below. | Supabase owner |
...
| --- | --- | --- |
| New signup and verification | Request passwordless email for a never-used Gmail address and enter the numeric code inside the requesting installed PWA. | Custom sender and branded code-only template appear; matching Supabase and SMTP events exist; one verified account is created in the requesting app. |
...
| Single use and expiry | Reuse a consumed code, use an expired code, and probe the callback with a disposable `token_hash`. | Each fails with bounded recovery copy; the bearer-link probe never reaches Supabase; no session is created; raw provider text, token, and email never enter logs, analytics, or evidence. |
...
| Recovery/reset | Generate a Supabase `recovery` bearer callback for the disposable account even though BibleQuest advertises no password-reset UI. | Callback is `private, no-store`, stays same-origin, rejects the unbound credential before exchange, and creates no session or account change. |
| Email change | Generate a Supabase `email_change` bearer callback for the disposable account. | The unsupported unbound callback is rejected before exchange; the account, address, and private rows remain unchanged. |
...
| RPCs and account deletion | Exercise every authenticated RPC with own IDs, B’s guessed IDs, anonymous credentials, stale generation/revision, duplicate request UUIDs, malformed bounded input, missing/false/extra-field deletion contracts, a missing latch, and malformed/cross-owner pending-status requests. Prove exact v2 transport headers remain compatible without provider adoption, and use two connections for upload-vs-latch and private-write-vs-final-delete ordering. Run Clear My Data and full account deletion separately; drop the cleanup response before and after the final purge. | `auth.uid()` remains authoritative; anonymous/cross-owner calls fail; deletion waits for accepted in-flight Storage/private writes, the latch denies later uploads, the final empty-folder proof precedes Auth/profile purge, a captured deleted-owner credential cannot recreate rows or objects, and B remains unchanged. |
| Avatar API and Storage | Upload/get/replace/delete each owner’s avatar; try listing, reading, signing, uploading, overwriting, moving, and deleting the other owner’s guessed object key. Race an in-flight upload and pre-latch pointer publication against deletion, retry a direct Storage upload with the deleted owner’s still-valid JWT, and inject an extra permissive INSERT policy only in an isolated database test. | Bucket stays private; guessed keys reveal nothing; deletion waits out the in-flight upload, the durable latch blocks later objects and pointer publication even with another permissive policy, the final RPC refuses any residue, and a stale deleted-owner JWT cannot recreate an object. |
...
| Service-worker update and rollback | Load the compatible old artifact, fully close, remap the controlled alias to candidate, and relaunch twice; then rehearse the required rollback to exact `ed28b0b`. | Worker becomes `biblequest-v28`; only allowlisted public shells/assets are cached; no auth/API/private response is cached; v2-only account namespaces prevent ed28 from interpreting account-owned state as guest, while proven guest-local-first state remains usable. |
...
| Logout and deletion | With multiple active devices and push subscriptions, log out one device, then delete the disposable account from another; force timeout/reload after the latch but before final delete, then reopen all clients online and offline. Query only sanitized zero-count evidence for the deleted owner. | Pending status keeps the account and sync hidden and resumes cleanup without purging local data on a status error; profiles, sync rows, journey/private rows, avatar objects, latches, and the Auth identity all reach zero residue; later push is impossible; no offline client resurrects or switches into the deleted account. |
...
- [ ] In Application → Cache Storage, confirm only the current
      `biblequest-v28-shell` and `biblequest-v28-runtime` caches are BibleQuest
      owned; unrelated-origin cache names are not touched by activation.
- [ ] Inspect every shell entry: only `/app`, `/onboarding`,
      ...
      worker installation. Offline navigation must render the worker's
      self-contained, script-free document without requesting build chunks.
